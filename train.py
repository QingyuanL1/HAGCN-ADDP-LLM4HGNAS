...
def train_epoch(model, optimizer, baseline, lr_scheduler, epoch, val_dataset, problem, tb_logger, opts):
    ...
    # When using multiple GPUs, you need to adjust the number of num_workers.
...
def train_batch(
    ...
    # Calculate loss
    cost = cost.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    # Logging
    if step % int(opts.log_step) == 0 and not opts.no_tensorboard and tb_logger is not None:
